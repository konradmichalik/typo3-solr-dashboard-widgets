...
```bash
# Clone repository
git clone https://github.com/konradmichalik/typo3-solr-dashboard-widgets.git
cd typo3-solr-dashboard-widgets
...
The DDEV setup ships a running Apache Solr container plus demo fixtures (indexed documents, scheduler task, search statistics) so the dashboard widgets have realistic data to render right after `ddev install`.

## Workflow

1. Fork the repository and create a feature branch from `main`.
2. Make your changes and ensure all linters and tests pass.
3. Use descriptive commit messages following the conventional commits format: `<type>: <description>` (e.g. `feat: add JVM memory widget`, `fix: handle missing Solr version header`).
